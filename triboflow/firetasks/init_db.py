...
def put_bulk_in_db(data, comp_params, db_file, high_level=True):
    ...
    if not prev_data:
        ...
        print(
            "Entry for mpid {} already found in {} collection. "
...
def put_inter_in_db(
    ...
    struct_2, mp_id_2, _, _ = material_data_for_db(
        data_2, comp_params, db_file, metal_thr=None
    )
    ...
    if not prev_data:
        nav.insert_data(
            functional + ".interface_data",
            {
                ...
                "formulae": (
                    struct_1.composition.reduced_formula,
                    struct_2.composition.reduced_formula,
                ),
                "pressure": pressure,
                "miller_indices": (data_1["miller"], data_2["miller"]),
                "comp_parameters": comp_params,
        ...
        print(
            f"Entry with name: {name} and external pressure {pressure} GPa\n"
