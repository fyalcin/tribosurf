...
class FT_StartSurfaceEnergyFromFile(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        slab, SG = slab_from_file(
            filename, mpid, functional, miller, db_file, high_level
        )

        inputs_list = get_surfen_inputs_from_slab(
            slab, SG, custom_id=custom_id
        )
...
class FT_RelaxSurfaceEnergyInputs(FiretaskBase):
    ...
    comp_params : dict
        Dictionary containing the computational parameters to be used.
    ...
    def run_task(self, fw_spec):
        ...
        for slab in inputs_list:
            ...
            for entry in slab.get("inputs"):
                ...
                # particular slab
                result_high = get_entry_by_loc(
                    nav_high, fltr_high, coll, loc
                ).get("output")
                if result_high:
                    continue
...
class FT_StartSlabOptOrientation(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        slab_dict = nav_high.find_data(
            f"{functional}.slab_data.LEO", {"mpid": mpid}
        )
...
class FT_WriteSurfaceEnergiesToDB(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        write_surface_energies_to_db(
            inputs_list, fltr, coll, db_file, high_level
        )
