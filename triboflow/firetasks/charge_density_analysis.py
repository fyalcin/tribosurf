...
def get_interface_region(interface):
        z_sub = [s.coords[2] for s in interface.sites
                 if s.properties['interface_label'] == 'substrate']
        z_film = [s.coords[2] for s in interface.sites
...
def make_charge_differences(interface, chgcar_int, chgcar_bot, chgcar_top):
        ...
        profile_inter_region = np.asarray([profile[i] for i in np.arange(chgcar_diff.dim[2])
                                    if z[i] >=zmin and z[i] <= zmax])
        
        abs_profile = abs_chgcar_diff.get_average_along_axis(2)
        abs_profile_inter_region = np.asarray([abs_profile[i] for i in np.arange(abs_chgcar_diff.dim[2])
                                    if z[i] >=zmin and z[i] <= zmax])
        ...
        try:
            ...
            rho_inter_region_abs = simpson(y=abs_profile_inter_region, dx=dz) / (zmax-zmin)
        
        return {'rho_classic': rho_inter_region,
                'rho_all': rho_total,
...
class FT_MakeChargeDensityDiff(FiretaskBase):
    ...
    def run_task(self, fw_spec):
        ...
        rho_dict = make_charge_differences(interface,
                                           chgcar_int, chgcar_bot, chgcar_top)
