    ...
    if sym:
        ...
        if not sto:
            ...
            layers_to_remove = int(
                pmg_layer_size
                * np.floor((sto_slab_layers - slab_layers) / ouc_layers)
            )
            target_layers = sto_slab_layers - layers_to_remove
            sto_slab = Shaper.resize(
                sto_slab, target_layers, vac_thickness, tol
            )
            # sto_slab = Shaper.remove_layers(sto_slab, layers_to_remove, tol, method='layers')
            sto_slab_input = generate_input_dict(
                sto_slab, "static", "sto_slab"
            )
            slab_static_input = generate_input_dict(
                slab, "static", "slab_static"
            )
        ...
        slab_tf_input = generate_input_dict(
            slab_tf, "relax", "slab_top_fixed_relax"
        )
        slab_bf_input = generate_input_dict(
            slab_bf, "relax", "slab_bot_fixed_relax"
        )
...
def update_miller_info(
    mpid, functional, max_index=2, db_file="auto", high_level=True
):
    fltr = {"mpid": mpid}
    nav_high = Navigator(db_file, high_level=True)
    bulk_conv = get_conv_bulk_from_mpid(
        mpid, f"{functional}.bulk_data", db_file, high_level
    )
    unique_hkl = get_symmetrically_distinct_miller_indices(
        bulk_conv, max_index
    )
    equiv_hkl = {}
    for hkl in unique_hkl:
        sym_eq_hkl = get_symmetrically_equivalent_miller_indices(
            bulk_conv, hkl
        )
...
def get_surfen_inputs_from_mpid(
    ...
    candidates = generate_candidate_slabs_from_mpid(
        mpid, sg_params, sg_filter, coll, db_file, high_level
    )
    inputs_list = [
        get_surfen_inputs_from_slab(c[0], c[1], tol, custom_id)
        for c in candidates
    ]
...
def move_result(
    tag, fltr, coll, loc, custom_dict={}, db_file="auto", high_level=True
):
    """
...
def write_surface_energies_to_db(
    inputs_list, fltr, coll, db_file="auto", high_level=True
):
    """
...
def generate_surfen_entries(fltr, coll, db_file="auto", high_level=True):
    ...
    for hkl, miller_data in data["miller_list"].items():
        ...
        params_dict = {
            k: v for k, v in miller_data[min_uid].items() if k in params
        }
        relaxed_structure = rel_str[min_uid]
        surfen_dict[hkl].update(
            {"relaxed_structure": relaxed_structure}, **params_dict
        )
...
def put_surfen_inputs_into_db(
    ...
    for slab_dict in inputs_list:
        ...
        nav.update_data(
            ...
            new_values={
                "$set": {
                    loc_slab + ".structure": slab.as_dict(),
...
def generate_candidate_slabs_from_mpid(
            ...
            *[[True, False], [True, False], ["max", None]]
            ...
            for hkl, slabs in slabs_dict.items():
                ...
                weight = sum(
                    [
                        1.5 * slab.num_sites
                        + slab.oriented_unit_cell.num_sites
                        for slab in slabs
                    ]
                )
    ...
    if method:
        slabs_list = [
            slab
            for slabs_by_hkl in slabs_dict.values()
            for slab in slabs_by_hkl
        ]
        ...
        if method == "bvs_threshold":
            ...
            filtered_slabs = [
                ...
                if slab.energy / min_bvs - 1 < bvs_tol
            ]
        ...
        slabs_list = [
            slab for slab_list in slabs_dict.values() for slab in slab_list
        ]
...
def calculate_surface_energy_gen(
    slab_dict, fltr, coll, db_file="auto", high_level=True
):
    """
...
def update_inputs_list(inputs_list, comp_params):
    ...
    for slab_data in inputs_list:
        ...
        for calc in inputs:
            ...
            uid_input = dict_to_hash(
                {**comp_params, "coords": struct_frac_coords}
            )
