...
def pbe_relax_with_static(struct, vis, name):
    ...
    fw_1 = OptimizeFW(
        structure=struct, name=name + "_PBE_relax", vasp_input_set=vis
    )
    fw_2 = StaticFW(
        structure=struct,
        vasp_input_set_params=vis_params,
...
def dynamic_relax_swf(
    inputs_list, wf_name="Dynamicaly generated Workflow", add_static=False
):
    ...
    for i in range(len(inputs_list)):
        ...
        if add_static:
            ...
            if vis.incar.get("METAGGA") in SCAN_list:
                ...
                fw_list.extend(fws)
                fw_links.update(links)
    WF = Workflow(fireworks=fw_list, links_dict=fw_links, name=wf_name)
