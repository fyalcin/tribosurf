...
for slab_dict in nav.find_many_data("PBE.slab_data", {}):
    try:
        slab_list.append({'mpid': slab_dict['mpid'],
                          'miller': slab_dict['miller'],
                          'slab': Slab.from_dict(slab_dict['relaxed_slab'])})
    except:
        pass

results = {'statistics': {'nr_of_interfaces': 0,
                          'failed_matches': 0,
...
for i, slab_combinations in enumerate(slab_comb_list):
    ...
    name = interface_name(slab_combinations[0]['mpid'],
    ...
    print('-'.center(100, '-'))
    IM_pmg = InterfaceMatcher(slab_combinations[0]['slab'],
                              slab_combinations[1]['slab'],
                              implementation="pymatgen",
                              **interface_parameters)
    IM_mpi = InterfaceMatcher(slab_combinations[0]['slab'],
                              slab_combinations[1]['slab'],
    ...
    if interface_pmg and interface_mpi:
        ...
        results[name] = {'pmg': {'interface': interface_pmg,
        ...
        results[name] = {'mpi': {'interface': interface_mpi,
                                 'area': interface_mpi.interface_properties['area'],
                                 'strain': interface_mpi.interface_properties['strain'],
                                 'lattice_abc': interface_mpi.lattice.abc},
                         'pmg': None
        ...
        results[name] = {'pmg': None,
                         'mpi': None
                         }
        
stats = results['statistics']
results['statistics']['percentage_matched_both'] = stats["both_methods_matched"]/stats["nr_of_interfaces"]*100
results['statistics']['percentage_matched_pmg'] = (stats["both_methods_matched"]+stats["pmg_only_matched"])/stats["nr_of_interfaces"]*100
results['statistics']['percentage_matched_mpi'] = (stats["both_methods_matched"]+stats["mpi_only_matched"])/stats["nr_of_interfaces"]*100